...
def prepare(subject):
    ...
    image_itk = convert_to_sitk(
        image
    )

    mask_itk = convert_to_sitk(
        mask
    )
...
def main():
    ...
    print(
        "GAN DELAYED RADIOMIC FEATURE EXTRACTION"
    )
    ...
    print(
        "GAN DELAYED RADIOMIC FEATURE EXTRACTION COMPLETE"
    )
    ...
    print(
        f"Subjects Processed : {success}"
    )

    print(
        f"Failed Subjects    : {failed}"
    )

    print(
        f"Features Extracted : {len(df.columns) - 1}"
    )
